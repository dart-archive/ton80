...
["", "Havlak.dart", , A, {
    ...
    createNode$1: function($name) {
      ...
      if (node == null) {
        ...
        if (($name & 0x3ffffff) === $name) {
          ...
          hash = t1._computeHashCode$1($name);
          bucket = rest[hash];
          if (bucket == null) {
            P._HashMap__setTableEntry(rest, hash, [$name, node]);
            t1._length = t1._length + 1;
            t1._keys = null;
          } else {
            index = t1._findBucketIndex$2(bucket, $name);
            if (index >= 0)
              bucket[index + 1] = node;
            else {
              bucket.push($name, node);
              t1._length = t1._length + 1;
              t1._keys = null;
            }
          }
  ...
  HavlakLoopFinder: {
    ...
    DFS$5: function(currentNode, nodes, number, last, current) {
      ...
      number[t1] = current;
      for (t1 = currentNode.get$outEdges(), t3 = currentNode.outEdges, lastid = current, target = 0; target < t1.length; ++target) {
        if (target >= t3.length)
          return H.ioore(t3, target);
        t4 = t3[target];
        t5 = t4.get$name();
        if (t5 >>> 0 !== t5 || t5 >= t2)
    ...
    findLoops$0: function() {
      var t1, t2, size, nonBackPreds, backPreds, number, header, types, last, nodes, i, w, nodeW, nv, v, nodePool, vi, t3, workList, n, x, iter, ydash, t4, t5, loop, np, node;
      t1 = this.cfg;
      t2 = t1.startNode;
      if (t2 == null)
        return 0;
      ...
      for (w = size - 1, t1 = this.lsg, t2 = t1.loops; w >= 0; --w) {
        ...
        if (nodePool.length > 0 || types[w] === 3) {
          ...
          t4 = [];
          t5 = [];
          loop = new A.SimpleLoop(t4, t5, t3, null, null, false, true, 0, 0);
          t4.push(nodeW);
          loop.header = nodeW;
          if (types[w] === 4)
