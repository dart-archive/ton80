...
["", "FluidMotion.dart", , M, {
  ...
  FluidField: {
    ...
    addFields$3: function(x, s, dt) {
      var t1, t2, i, t3;
      for (t1 = this.size, t2 = x.length, i = 0; i < t1; ++i) {
        if (i >= t2)
          return H.ioore(x, i);
        t3 = x[i];
        if (i >= s.length)
    ...
    set_bnd$2: function(b, x) {
      var t1, t2, t3, t4, t5, t6, i, t7, t8, j, maxEdge;
      if (b === 1) {
        for (t1 = this.width, t2 = this.rowSize, t3 = x.length, t4 = this.height, t5 = (t4 + 1) * t2, t6 = t4 * t2, i = 1; i <= t1; ++i) {
          t7 = i + t2;
          if (t7 >= t3)
            return H.ioore(x, t7);
          t7 = x[t7];
    ...
    advect$6: function(b, d, d0, u, v, dt) {
      var t1, Wdt0, t2, Hdt0, Wp5, Hp5, t3, t4, j, pos, i, x, y, i0, i1, j0, s1, t10, t00, row1, row2, t5, t6, t7, t8;
      t1 = this.width;
      Wdt0 = dt * t1;
      t2 = this.height;
      Hdt0 = dt * t2;
      Wp5 = t1 + 0.5;
      Hp5 = t2 + 0.5;
      for (t3 = this.rowSize, t4 = d0.length, j = 1; j <= t2; ++j) {
        pos = j * t3;
        for (i = 1; i <= t1; ++i) {
          ++pos;
          if (pos >= u.length)
          ...
          d[pos] = (1 - s1) * (t00 * t5 + t10 * t6) + s1 * (t00 * t7 + t10 * t8);
        }
      }
      this.set_bnd$2(b, d);
    },
    project$4: function(u, v, p, div) {
      var t1, t2, h, t3, t4, t5, j, row, previousRow, prevValue, nextValue, nextRow, currentRow, i, t6, t7, t8, t9, wScale, hScale, currentPos, prevPos, nextPos, prevRow;
      t1 = this.width;
      t2 = this.height;
      h = -0.5 / Math.sqrt(t1 * t2);
      for (t3 = u.length, t4 = v.length, t5 = this.rowSize, j = 1; j <= t2;) {
        row = j * t5;
        previousRow = (j - 1) * t5;
        prevValue = row - 1;
        nextValue = row + 1;
      ...
      for (t6 = p.length, j = 1; j <= t2;) {
        ...
        for (i = 1; i <= t1; ++i) {
          ...
          u[currentPos] = t7 - wScale * (t8 - p[prevPos]);
          if (currentPos >= t4)
          ...
          t7 = p[nextRow];
          ++prevRow;
          if (prevRow < 0 || prevRow >= t6)
            return H.ioore(p, prevRow);
          v[currentPos] = t8 - hScale * (t7 - p[prevRow]);
        }
      }
      this.set_bnd$2(1, u);
      this.set_bnd$2(2, v);
